...
从业务代码自动梳理 **业务场景 → 功能 → 条件分支用例**，生成研发自测用例 Excel（含覆盖核算、规则缺口审查、风险提示），支持主流语言。
...
- 「覆盖说明」sheet：覆盖统计（入口/判定点）、判定点→用例追溯矩阵、标注区（缺口 / 无法静态验证 / 规则缺口）

## 核心能力

- **机器枚举判定点**作覆盖率硬分母——漏测亮红灯，不虚增
- **条件展开**：复合条件拆分、边界值、状态机、调用链、嵌套不笛卡尔积
- **需求驱动**：附带需求/契约时，预期结果对照需求、并标出"该有却没有"的规则缺口
